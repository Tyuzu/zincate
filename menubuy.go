...
func BroadcastMenuUpdate(placeId, menuId string, remainingMenus int) {
	...
	channel := GetUpdatesChannel(placeId)
	select {
	case channel <- update:
...
func buyxMenu(w http.ResponseWriter, request MenuPurchaseRequest, requestingUserID string) {
	...
	var menu Menu // Define the Menu struct based on your schema
	...
	_, err = menuCollection.UpdateOne(context.TODO(), bson.M{"placeid": placeId, "menuid": menuID}, update)
	if err != nil {
		http.Error(w, "Failed to update menu stock", http.StatusInternalServerError)
		return
	}

	SetUserData("menu", menuID, requestingUserID)
